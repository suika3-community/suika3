 ...
 */
static bool load_cached_glyph(int slot, int codepoint, int size, pixel_t color, int outline_width, pixel_t outline_color);
static bool isgraph_extended(const char **mbs, uint32_t *wc);
static int translate_font_type(int font_type);
static bool draw_emoji(struct draw_msg_context *context, const char *name, int *w, int *h);
...
s3_get_string_height(int font_type, int font_size, const char *mbs)
{
	...
	while (*mbs != '\0') {
		...
		/* Get the character width. */
		h = get_glyph_height(font_type, font_size, c);
		if (h > h_max)
			h_max = h;
	...
	bool use_tategaki)
{
	...
	if (s3_is_quoted_serif(msg) && !use_tategaki)
		context->left_margin += get_glyph_width(font, font_size, U32C('　', 0x3000));
}
